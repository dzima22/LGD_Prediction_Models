...
import numpy as np
from modules.config import (features_model1,features_model2,Random_state,Test_size,pbounds_xgboost,eval_metric_xgboost,cv_xgboost,epochs_nn_model)

def logit_for_PC(df):
    X_train1, X_test1, y_train1, y_test1 = train_test_split(df[features_model1], df['PC'], test_size=Test_size, random_state=Random_state)
    X_train2, X_test2, y_train2, y_test2 = train_test_split(df[features_model2], df['PC'], test_size=Test_size, random_state=Random_state)
...
def logit_model_adasyn(df):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_scaled[features_model1],df_scaled['PC'],test_size=Test_size,random_state=Random_state)
...
def glm_rr_models(df):
    ...
    glm_full = sm.GLM(rr_target.loc[train_idx], sm.add_constant(df.loc[train_idx, features_model1]), family=sm.families.Binomial())
    res_full = glm_full.fit()
    glm_small = sm.GLM(rr_target.loc[train_idx], sm.add_constant(df.loc[train_idx, features_model2]), family=sm.families.Binomial())
    res_small = glm_small.fit()
    return {"model1": {"model":  res_full,"X_test": sm.add_constant(df.loc[test_idx,  features_model1]),"y_test": rr_target.loc[test_idx]},
            "model2": {"model":  res_small,"X_test": sm.add_constant(df.loc[test_idx,  features_model2]),"y_test": rr_target.loc[test_idx]}}
def xgb_models(df):
    train_idx, test_idx = train_test_split(df.index, test_size=Test_size, random_state=Random_state)
    ...
    def xgb_cv_model1(n_estimators, gamma, learning_rate, colsample_bytree,reg_lambda, reg_alpha, subsample, min_child_weight):
        ...
        scores = cross_val_score(model, X_train1, y_train1, cv=cv_xgboost, scoring='roc_auc')
        return scores.mean()
    optimizer1 = BayesianOptimization(f=xgb_cv_model1, pbounds=pbounds_xgboost, random_state=Random_state)
    ...
    model1 = XGBClassifier(**best_params_model1,scale_pos_weight=scale_pos_weight1,use_label_encoder=False,eval_metric='logloss',random_state=Random_state)
    ...
    def xgb_cv_model2(n_estimators, gamma, learning_rate, colsample_bytree,reg_lambda, reg_alpha, subsample, min_child_weight):
        model = XGBClassifier(n_estimators=int(n_estimators),gamma=gamma,
            ...
            eval_metric=eval_metric_xgboost,
            random_state=Random_state)
        scores = cross_val_score(model, X_train2, y_train2, cv=cv_xgboost, scoring='roc_auc')
        return scores.mean()
    optimizer2 = BayesianOptimization(f=xgb_cv_model2, pbounds=pbounds_xgboost, random_state=Random_state)
    ...
    model2 = XGBClassifier(**best_params_model2,scale_pos_weight=scale_pos_weight2,use_label_encoder=False,eval_metric=eval_metric_xgboost,random_state=Random_state)
...
def xgb_regression_models(df):
    X1_train, X1_test, y1_train, y1_test = train_test_split(df[features_model1],  df['RR'], test_size=Test_size, random_state=Random_state)
    X2_train, X2_test, y2_train, y2_test = train_test_split(df[features_model2],  df['RR'], test_size=Test_size, random_state=Random_state)
    def xgb_cv_model1(n_estimators, gamma, learning_rate, colsample_bytree,
        ...
        model = XGBRegressor(n_estimators=int(n_estimators),gamma=gamma,learning_rate=learning_rate,colsample_bytree=colsample_bytree,reg_lambda=reg_lambda,reg_alpha=reg_alpha,
            subsample=subsample,min_child_weight=min_child_weight,random_state=Random_state)
        scores = cross_val_score(model, X1_train, y1_train, cv=cv_xgboost, scoring='neg_root_mean_squared_error')
        return scores.mean()
    def xgb_cv_model2(n_estimators, gamma, learning_rate, colsample_bytree,reg_lambda, reg_alpha, subsample, min_child_weight):
        model = XGBRegressor(n_estimators=int(n_estimators),gamma=gamma,learning_rate=learning_rate,colsample_bytree=colsample_bytree,reg_lambda=reg_lambda,reg_alpha=reg_alpha,subsample=subsample,
            min_child_weight=min_child_weight,
            random_state=Random_state)
        scores = cross_val_score(model, X2_train, y2_train, cv=cv_xgboost, scoring='neg_root_mean_squared_error')
        return scores.mean()
    optimizer1 = BayesianOptimization(f=xgb_cv_model1, pbounds=pbounds_xgboost, random_state=Random_state)
    optimizer1.maximize(init_points=5, n_iter=25)
    optimizer2 = BayesianOptimization(f=xgb_cv_model2, pbounds=pbounds_xgboost, random_state=Random_state)
    ...
    best_params_model2 = optimizer2.max['params']
    best_params_model2['n_estimators'] = int(best_params_model2['n_estimators'])
    model1 = XGBRegressor(**best_params_model1, random_state=Random_state)
    model1.fit(X1_train, y1_train)
    model2 = XGBRegressor(**best_params_model2, random_state=Random_state)
...
def train_lgd_nn_models(df, epochs=epochs_nn_model):
    ...
    class Net(nn.Module):
        ...
        def forward(self, x):
            return self.model(x)
    X_rr_train, X_rr_test, y_rr_train, y_rr_test = prepare_data(features_model1, 'RR')
    X_pc_train, X_pc_test, y_pc_train, y_pc_test = prepare_data(features_model2, 'PC')
    y_lgd = df['lgd'].values
    _, _, y_lgd_train_np, y_lgd_test_np = train_test_split(
        df[features_model1].copy(), y_lgd, test_size=Test_size, random_state=Random_state)
